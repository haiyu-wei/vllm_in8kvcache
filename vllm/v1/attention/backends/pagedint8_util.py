...
def int8_attention_varlen_simulate(
    ...
    # 构建KV的累积序列长度（cu_seqlens_k），适配变长序列
    ...
    # 初始化批次化张量（batch, max_seq_len, num_heads, head_dim）
    # 假设是float32 实际应该用int8
    q_batch = torch.zeros(
    ...
    # 适配GQA/MQA：KV头数 < Q头数时，重复KV头维度
    if num_heads != num_kv_heads:
        g = num_heads // num_kv_heads
        k_batch = repeat(k_batch, "b s h d -> b s (h g) d", g=g)
        v_batch = repeat(v_batch, "b s h d -> b s (h g) d", g=g)

    # # 模拟INT8计算：先做int8矩阵乘法，再缩放（而非先反量化再乘法）
    # scores = (Q_i8 @ K_i8^T) * (q_scale * k_scale / sqrt(d))
    # 输入 1（q_batch）：b t h d → [batch, seqlen_q, heads, head_dim]
    # 输入 2（k_batch）：b s h d → [batch, seqlen_k, heads, head_dim]
    # 输出（scores）   ：b h t s → [batch, heads, seqlen_q, seqlen_k]
    d = head_dim
    ...
    # 如果scale只有一个值，扩展到每个头；如果KV头数少于Q头数，重复KV的scale
    if q_scale_.numel() == 1:
    ...
    # 计算最终缩放因子：(q_scale * k_scale) * softmax_scale（softmax_scale=1/sqrt(head_dim)）
    # 1/sqrt(d) * q * k^T
    scale_scores = (
    ...
    scores = scores.float() * scale_scores.to(scores.device)    # 还原注意力分数

    # 软裁剪：限制分数范围，避免Softmax数值不稳定
    if logits_soft_cap > 0:
        scores = (scores / logits_soft_cap).tanh() * logits_soft_cap
    
    # ALiBi位置偏置：无需位置编码，通过线性偏置实现位置感知
    ...
    # 拼接变长序列输出：按cu_seqlens_q提取每个样本的有效输出
    out_flat = []
    for b in range(batch_size):
        q_start = cu_seqlens_q[b].item()
        q_end = cu_seqlens_q[b + 1].item()
